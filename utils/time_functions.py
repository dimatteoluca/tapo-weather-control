...
def wait_until(target_time):
    try:
        ...
        logger_tw.info("Waiting until:           %s", target.strftime('%H:%M'))
        time.sleep(delta.seconds + 1)
    except Exception as e:
        logger_tw.error(f"Error occurred while waiting: {str(e)}")
...
def wait_until_next_hour():
    try:
        ...
        logger_tw.error(f"Error occurred while waiting until next hour: {str(e)}")
